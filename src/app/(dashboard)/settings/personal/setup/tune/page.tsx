...
import { OnboardingTuneClient } from "@/app/(dashboard)/onboarding/tune/ui";

export default async function PersonalSetupTunePage({
  searchParams,
}: {
  ...
  if (!runId) redirect("/settings/personal/setup/scan");
  ...
  return <OnboardingTuneClient runId={run.id} status={run.status} questions={questions} mode="setup" />;
}
