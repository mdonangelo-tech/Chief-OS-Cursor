...
export default async function PersonalSetupIntroPage() {
  ...
  if (lastComplete) {
    ...
    return (
      <div className="max-w-2xl space-y-6">
        <div>
          <h1 className="text-2xl font-semibold">Personal context</h1>
          <p className="text-muted-foreground mt-1">Your setup is ready. You can refine this anytime.</p>
          <p className="text-muted-foreground text-sm mt-2">
            Last updated <LocalTime value={lastComplete.completedAt} />
          </p>
        </div>
              ...
              return (
                <li key={a.id} className="flex flex-wrap items-center gap-2">
                  ...
                  {p?.includeInOnboarding === false && (
                    <span className="text-xs text-muted-foreground">(excluded from scan)</span>
                  )}
                </li>
              );
            })}
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <h1 className="text-2xl font-semibold">Personal context setup</h1>
        <p className="text-muted-foreground mt-1">
          A quick setup to learn your goals, scan email + calendar, and personalize your Brief.
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-border/10 bg-surface/60 p-5 space-y-3 shadow-soft">
        ...
        <ul className="text-sm text-muted-foreground space-y-1 list-disc pl-5">
          <li>Confirm which accounts to include</li>
          <li>Set what you’re optimizing for</li>
          <li>Run a scan and get a first set of recommendations</li>
        </ul>
      </div>
      ...
      {accountsCount === 0 ? (
        <div className="rounded-2xl border border-border/10 bg-surface/60 p-5 space-y-3 shadow-soft">
          ...
          <Link
            href="/api/connect-google?returnTo=/settings/personal/setup"
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/settings/personal/setup/accounts"
            className="inline-block rounded-xl bg-accent px-4 py-2 text-accent-foreground font-medium hover:opacity-90"
          >
            Continue
          </Link>
